...
def build_debug_message(report: Dict) -> str:
    ...
    lines = ['🛠️ <b>Resumen debug bot casas v5</b>', '']
    for portal in report.get('sources', []):
        lines.append(
            f"• <b>{html.escape(portal['name'])}</b>: enabled={portal['enabled']}, extraídos={portal['raw_count']}, válidos={portal['valid_count']}, nuevos/cambios={portal['notify_count']}, errores={portal['error_count']}"
        )
        if portal.get('final_url'):
            lines.append(f"  ↳ URL final: {html.escape(portal['final_url'][:120])}")
        if portal.get('page_title'):
            lines.append(f"  ↳ Título: {html.escape(portal['page_title'][:120])}")
        if portal.get('block_signals'):
            lines.append(f"  ↳ Bloqueos: {html.escape(', '.join(portal['block_signals'][:5]))}")
